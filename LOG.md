...
- `getLegalDropSquares`は有効持駒種につき81マス、`validateDrop`は占有→行き所→二歩→盤面複製→自玉王手→歩打ち詰めの順。rootの盤上駒移動と持駒打ちを呼出回数・合計・単一最大で分け、打ちは駒種別の候補・合法・最初の却下理由を記録。盤面複製、王手、歩打ち詰めの工程回数を常に、工程時間を明示ON時のみ記録する。root包含時間から既存の排他的な盤上移動・打ちを差し引き残余を保存し、駒種別打ちとその内工程は二重加算しない。定義と再実行方法は`docs/benchmarks/root-legal-breakdown-method.md`。
- 少数局面`g3-p55`・`g1-p93`で工程時計OFF/ONを各1ウォームアップ＋2本測定した。root中央値は件数のみで36.68/45.36ms、詳細時計で36.29/45.74ms。差は小標本の順序・実行時変動を含み、純粋な時計負荷とは断定しない。両パイロットのJSONL/Markdownを保存し、監査した。
- 主測定は保存棋譜4局面`g1-p115`、`g3-p55`、`g4-p89`、`g1-p93`を各2ウォームアップ＋5本測定、OFF/ON交互順、計56サンプル（本測定40）。Windows 10.0.26200 x64、i7-14650HX、Node v24.20.0、100ms、最大深さ4、標準評価・着手順、静止追加1手、同期直列。測定開始時のHEADは上記main、`dirty=true`（計測コード・成果物が未コミット）。本測定は40/40が100ms超、40/40がfallback、OFF/ONの選択手と深さは20/20組一致。呼出中央値は局面別OFF/ONで`g1-p115` 100.42/100.31ms、`g3-p55` 100.47/100.30ms、`g4-p89` 100.24/100.37ms、`g1-p93` 100.69/101.11ms。ONのroot中央値は順に49.32/35.84/39.12/45.88ms、打ちは42.96/29.46/32.83/39.54ms。診断ONの20本を通算してroot 856.46ms、持駒打ち727.65ms、盤面複製工程709.63ms・4545回。持駒打ち単一最大は10.50ms、root単一最大50.69ms。これらの比率は今回の局面・診断設定に限る。
- PR #158で打ち単一最大97.17msが出た`g3-p117`も2ウォームアップ＋5本測定し、同一局面を確認。今回の打ち単一最大10.06ms、root単一最大43.21msで長い尾は再現しない。さらに元棋譜4局480手を、各手に入力保護Proxyを付けて探索し、元の手で進める連続再生を実施。全対象のpositionKey一致、480/480呼出が100ms超、root最大73.08ms、打ち単一最大18.76ms。対象のrootは38.93～53.71ms。探索結果で進める実対局とは条件が異なるが、保存局面の同一性と連続呼出しを確認した。保護なしの予備測定は元対局条件を欠くため破棄して再測定した。
- PR #158の`g1-p93`では期限確認間隔415.56ms中、root持駒打ち合計360.60ms、盤上移動54.01ms、持駒打ち単一最大85.14msだった。複数回の打ち生成の累積が区間の大部分を占め、85.14ms一呼出しのみで区間全体を説明しない。今回の同一機・同一局面ではその時間帯を再現できず、当時の各工程内部の時間・OS/JIT/GC等の寄与は未確定。今回の詳細計測から、通常観測されたroot持駒打ち費用の主因候補は盤面複製。次の最適化候補は複製経路の精査だが、このPRでは変更も改善量推定もしない。PR #158の4局はいずれも120手上限打切で勝敗・棋力への影響は推定しない。
- 主測定・補助局面・両パイロットの新監査、保存棋譜4局480手の連続再生監査、旧`long-tail-v1`の4局522標本監査に成功。合法手内容・順序一致と入力不変性を測定器とルールテストで確認。関連4ファイル90件のテスト成功。駒種別時間からソートを除く時計境界修正後、全データを再取得して再監査し、`npm run check`でlockfile、型検査、63ファイル1743/1743テスト、build（1756 modules）成功。`git diff --check`成功。sandbox内の`tsx`は`spawn EPERM`となり、許可された実行経路で測定・テスト・監査を実施した。

## [2026-09-24 JST] 100ms探索の長い連続対局とGC・ヒープ診断

- 基点はマージ済みPR #156の`main`=`cace50b`。既存の測定・監査CLI、保存棋譜、診断、self-play、作業指示を確認し、`test/timed-long-tail-diagnostics`で測定だけを実装した。通常のルール、評価、着手選択、探索設定は変更せず、長時間測定を通常CIに追加しない。
- PR #156の診断は注入時計の開始値と`performance.now()`の工程時刻を混用し、呼出後の`finish`を`api-other`に含めていた。開始・工程・deadline通過・終了を注入時計に統一し、API内の終了時計で診断を確定した。異なる値を返す偽時計で期限通過工程、工程時間とAPI内時間の整合を検証した。診断ONの追加時計読みはdeadline判定と結果を動かし得るため、OFF/ON対応測定を付けた。新CLIは`v2`、PR #156の保存済みJSONLは呼出後まで含む`v1`として変更せず区別する。API内実時間、呼出直前から戻り直後までの実時間、API内の排他的工程以外である`api-other`の定義をCLI設定行、報告、`docs/benchmarks/timed-long-tail-method.md`に記した。
- 既存の入力保護Proxyと履歴付きself-playを再利用し、最大手数付き4局を同期直列で測定する研究CLIを追加。各手の局面キーSHA256、結果由来・完了深さ・時間切れ、API内と呼出全体の実時間、100ms超過、工程診断、GCイベントの時刻・所要時間・種別、前後時刻付きheapUsedをJSONLに保存する。GC通知は同期探索後のイベントループで回収し、`performance`時刻区間の交差で手に結び付ける。観測不能時は理由を残す。前後ヒープは探索中の最大値ではない。最後の期限確認から次の確認までの間隔と、rootの駒別移動生成・持駒種別別打ち生成、静止探索の評価・王手・合法手・並べ替え・着手適用の単一呼出し最大値を区別して記録する。監査CLIは棋譜、局面ハッシュ、結果、件数、時間内訳、GC重複、集計を再計算する。
- 最終生データは`docs/benchmarks/timed-long-tail-main-20260924.jsonl`、集計は同名`.md`。探索コードSHA`479a13938a925701a6eba73f449d51940fdfc545`、実行開始時`dirty=false`、Windows 10.0.26200 x64、i7-14650HX、Node v24.20.0、100ms・最大深さ4・標準評価／並べ替え・静止追加1手。2ペア4局、各120手で上限打切、全480手。保存棋譜の`g1-p115`、`g3-p55`、`g4-p89`は各OFF/ONで2ウォームアップ・5本測定。全522標本（うち本測定510）を監査CLIで再計算した。
- 本測定510件中fallback481件、100ms超510件、105ms超368件。元の`g1-p115`の333.57ms事象は今回再現し、OFFで最大433.80ms、ONで最大411.96ms。`g3-p55`はOFF263.60～305.06ms、ON255.26～273.03ms、`g4-p89`はOFF267.24～292.01ms、ON269.34～286.12ms。いずれもこの5本はfallbackで、ONではroot持駒打ち生成中に100msを通過した。最長の期限確認間隔は`g1-p93`の415.56ms（root合法手生成）、root内部の最長単一処理は`g3-p117`の持駒打ち生成97.17ms、静止探索内部は`g3-p40`の合法手生成40.66ms。`g1-p115`の最大433.80msにはGC1件、同局面のON最大411.96msにはGC0件が重なった。GC観測228件、欠測0件。時間的重複だけで原因と断定しない。最初・最後のヒープ標本は36,965,568→159,849,208 bytesで、処理中のピークではない。
- 保存棋譜局面15組の診断ON-OFFの実時間差は中央値-0.64ms、範囲-47.19～+19.83ms。選択手・完了深さの相違は各0組だった。順序と実行時変動があり、診断負荷をこの差だけから精密に分離できない。4局は全て120手上限で、勝敗や棋力への影響の証拠にしない。次の修正候補は、長いroot合法手生成の持駒打ち判定と、静止探索の合法手生成の費用を別計測で確認すること。最適化・棋力比較は本変更に含めない。
- 検証: 型検査成功、全63テストファイル1741件成功（追加の偽時計テスト後は関連ファイル4件成功）、build成功、旧PR #156 JSONLの監査7局面196標本成功、新JSONLの監査4局522標本成功、`git diff --check`成功。sandbox内の`esbuild`起動は`spawn EPERM`となったため許可された実行経路でテスト・測定・監査・ビルドを行った。

## [2026-09-24 JST] 深さ1と100ms超過の工程診断

- 基点`main`=`9e336e6`、作業ツリー空から`test/timed-depth-one-diagnostics`を作成。PR #150の棋譜再生・API実時間、#152のroot合法手生成前の時計と協調的deadline、#154の全480手棋譜を確認した。対象は探索APIへの任意診断と測定CLI、集計・監査、文書。着手選択、ルール、評価、既定設定は変更しない。
- 7局面を棋譜の直前まで合法手適用で再生し、履歴を保つ。root合法手、通常合法手、並べ替え、SEE、着手適用、静止探索、その他を入れ子除外で計測する。`crossedDeadlinePhase`と`interruptedPhase`を区別する。標準手順のSEEは0回。
- スモーク1局面8サンプルで固定深さの手・評価・PV一致を確認し、監査で履歴ハッシュ・集計を再現した。最初のtsx実行はsandboxの`spawn EPERM`で停止し、許可された経路で成功した。通常CIに長時間の測定を追加しない。
- 最初のコードSHA`900cdb9`での測定は再生局面を通常状態のまま探索へ渡したため、実対局の`protectSearchInput`が作る再帰Proxy負荷を落としていた。固定深さ1の費用が元棋譜と整合せず、主結果には採用しない。既存の入力保護関数を計測器から利用し、スモークと型検査を再実行してコードSHA`f7ae3d2ffe4c460aa3ef1f2dc09deb12d4301caa`を確定。クリーンな独立作業ツリー・`config.dirty=false`で7局面196サンプルを同期直列で再測定した。Windows 10.0.26200 x64、i7-14650HX、Node v24.20.0。各条件2ウォームアップ・5本測定。保存済みJSONLを`audit:timed-depth-one`で棋譜・状態ハッシュ・排他的時間・集計まで再照合した。
- 100ms探索の深さ1完走は序盤5/10、中盤5/10、後半5/15で診断OFF/ON一致。診断ONの期限通過はroot合法手10/35、静止探索16/35、通常合法手6/35、通常着手適用3/35。`g3-p55`と`g4-p89`のroot合法手生成中に100msを越え、実時間はそれぞれ243～263msと350～399ms。元の最大333.57ms局面`g1-p115`は5/5 fallbackだが実時間100.3～102.7msで、元の単発超過の内部位置は不明。固定深さ1は`g1-p115`で診断ON771～808ms、静止探索569～597ms／314回、通常着手適用152～161ms。深さ1の費用と期限超過の工程を分けて報告する。
- 診断OFF/ONの同一局面・同一反復の実時間差は固定深さ1中央値-0.82ms（-697.35～+39.94）、100ms中央値+0.03ms（-72.36～+67.12）。固定深さ70回の手・評価・PV一致、100msの35組中1組は完了深さ差1、選択手は全組一致。大きな揺れがあり計測負荷は精密に推定できない。診断OFFでも中盤最大286.67ms、後半最大422.29ms。後続で長い連続対局とGC・ヒープ、root合法手／静止探索内部の最大中断不能区間を記録する。
- `npm run check`は63テストファイル1740テスト・型・ビルド成功。既存のA/B生成Markdownに表と本文の空行を追加して表示崩れを修正した。

## [2026-09-24 JST] 時間制限付き探索の実対局fallback測定

- PR #152後の`main`=`eeae079c33c4f3a40781a0dee9d751a5feb20ccf`、作業ツリー空を確認し、`feat/timed-fallback-self-play-measurement`で実装。既存self-playの合法手・終局判定、先後交代ペア、PR #150のAPI内／呼び出し全体の時間区別を再利用。探索本体、評価、対局ルール、通常設定は変更しない。
- `selfPlayGame.ts`に任意の呼び出し実経過時間の観測フィールドを追加。`scripts/benchmarks/timedFallbackSelfPlay.ts`は同設定のA/Bを既存ペア実行器に渡し、偶数ペアだけローカルA/Bを交換してAB→BA／BA→ABの実行順を交互化。各着手に参加者、指定時間、API内時間、実時間、深さ、timeout、sourceを保持し、fallbackで評価値やPVを補作しない。JSONLは`config`→各`game`→`summary`→`end`。参加者・局単位の着手分母、fallback回数・率、20手帯、深さ、超過幅、勝敗／引分／打切／失敗を別集計。CLIは明示出力先に排他的作成し、同名Markdownを生成。audit CLIは生データを合法手として全再生して集計を再現する。
- 初回の成果物は実装前mainを`head`に記録し`dirty:true`だったため、確定した計測コード`dc8b2ec4fbc3c7922a2eb7c723260c24cfe249e1`をクリーンな作業ツリーから再実行した。3条件をリポジトリ外の一時ディレクトリへ同期直列に出力し、各JSONLの`config.head`がこのSHA、`dirty:false`、設定が指定条件どおりであることを確認した。通常実行経路での最初のスモークは計測前の`spawnSync git EPERM`で停止し、空の一時ファイルだけを除去して許可経路で再実行した。計測コード・対局ルール・評価・探索の着手選択は変更していない。
- 再測定スモーク: 100ms・1ペア2局・4手上限、8合法着手、fallback 0、打切2、実行1.0秒。主条件: 100ms・2ペア4局・120手上限、480着手、実行92.9秒。fallback A 221/240（92.1%）、B 222/240（92.5%）、計443/480（92.3%）。4局すべて120手打切、勝敗・引分0。1～20手60/80、21～40手70/80、41～60手76/80、61～80手80/80、81～100手77/80、101～120手80/80。呼び出し実時間は100ms超480/480、105ms超311/480、200ms超159/480、最大333.57ms。API内最大333.51ms。
- 再測定参考条件: 1,000ms・1ペア2局・40手上限、80着手、実行82.0秒。fallback A 6/40（15.0%）、B 6/40（15.0%）、計12/80（15.0%）。1～20手0/40、21～40手12/40。2局とも打切、勝敗・引分0。呼び出し実時間は1,000ms超80/80、1,005ms超10/80、1,100ms超0、最大1,009.32ms。API内最大1,009.26ms。3条件のJSONL全568着手の合法性と保存済み集計をauditで再確認し、6ファイルをハッシュ照合して既存成果物へ反映した。
- `npm run check`はlockfile検証、TypeScript型検査、62ファイル1738/1738テスト、Vite build成功。通常サンドボックスではVitest起動時にesbuild `spawn EPERM`となるため許可経路で実行した。既存jsdomのnavigation通知はテスト失敗ではない。100msの全局にfallbackがあり4/4局打切、1,000msも2/2局打切なので成績との関連を判定できない。旧版比較は深さ1完走による実時間不一致があり、この計測には含めず、隔離した実時間統制対局を別課題とする。
...
- 変更前の深さ1診断（中盤30合法手×3回、追加1手静止探索）は、root合法手生成0.40ms、候補手前の全局面複製5.20ms、着手実行4.57ms、末端評価2.53ms、探索全体35.87ms。全局面複製は着手実行と同程度の確かな重複費用だった。各処理を別に測る診断値なので合計を探索全体の内訳率とは扱わない。変更後の同じ深さ1診断は探索全体27.33ms、通常探索は変更前12.19ms→変更後8.23ms。個別測定での`clone`欄は診断用の独立呼び出しであり、変更後の探索本体では実行しない。
- 測定PCはWindows x64、Intel Core i7-14650HX、Node 24.20.0、npm 11.17.0。局面は平手30手、中盤（公開合法手を20手再生）30手、構成終盤の王手回避11手。各モード（通常・静止追加1手）×局面×100/1,000msについて同一プロセス内で2ウォームアップ・5本測定を直列実行し、他のテスト・build・測定は並列に実行していない。全対局測定は行っていない。生ログは`docs/benchmarks/time-limit-overrun-before-{plain,q1}.jsonl`と`time-limit-overrun-after-{plain,q1}.jsonl`、診断と固定深さ比較も同ディレクトリの`before/after-{profile,profile-q1,fixed,fixed-q1}.jsonl`にある。再実行は`npx tsx scripts/measure-time-limit-overrun.ts timed`または`timed-q1`（診断`profile`/`profile-q1`、固定`fixed`/`fixed-q1`）。

| 条件 | 平手100ms | 平手1,000ms | 中盤100ms | 中盤1,000ms | 王手回避100ms | 王手回避1,000ms |
| --- | --- | --- | --- | --- | --- | --- |
| 通常・変更前の完了深さ | 3×5 | 3×5 | 1×5 | 2×5 | 1×1、2×4 | 2×5 |
| 通常・変更後の完了深さ | 3×5 | 3×4、4×1 | 2×5 | 3×5 | 2×5 | 2×5 |
| 静止追加1手・変更前の完了深さ | 2×5 | 2×5 | 1×5 | 2×5 | 1×5 | 1×5 |
| 静止追加1手・変更後の完了深さ | 2×5 | 2×5 | 1×5 | 2×5 | 1×5 | 1×5 |

- 全本測定の実経過時間、API内経過時間、完了深さ、`timedOut`、選択手、反復時間は上記JSONLに各回別で記録した。通常・中盤100msの変更前は実時間100.39～101.16ms、変更後100.13～100.65ms。静止追加1手・中盤100msは変更前100.27～100.66ms、変更後100.28～103.07msで、完了深さの改善は確認できなかった。1,000msの多くは期限で中断され、実時間は概ね1,000～1,003ms。通常・平手の変更後1回のみ深さ4を633.26msで完了して`timedOut=false`。OS負荷・JIT・GCの揺れがあり、これを一般的な短縮幅と見なさない。
- 固定深さ1/2×3局面×通常/静止追加1手の変更前後12行は完全一致し、合法な選択手、評価値、PV、評価内訳、通常探索統計を保った。既存の注入時計テストは深さ1保証、深さ2以降の事前・再帰中・終了直後の期限、未完了反復の破棄、`timedOut`、`completedDepth`、`iterations`、`total*`統計を確認する。追加テストは履歴を持つ凍結局面を公開着手APIと時間制限探索へ渡し、入力不変と静止探索付き深さ1結果・合計統計を検査する。
- 残る制約: 深さ1は期限から除外される。深さ2以降も同期処理中の協調的確認なので、100ms/1,000msは厳密な上限ではない。今回の3局面では深さ1の大幅超過は再現しなかった。実対局の長い履歴や静止探索の複雑な局面での超過を完全に解消した証拠はなく、厳密な上限が必要なら深さ1保証・結果型・Worker/UI/対局記録を一緒に見直す設計変更が必要。
- `npm run check`は終了0。lock検証、TypeScript型検査、全60ファイル・1,726テスト、Vite本番buildが成功した。関連4ファイルの先行テストも168件成功。`git diff --check`とPR CIの最終結果はPR作成後に確認する。
...
- 序盤3局面だけでは先手番・初期段階へ偏り、PR #142の5秒・10秒観測を一般化する材料にならないため、型付き`phase`（opening/middlegame/endgame）と期待`sideToMove`を各シナリオへ追加した。既存ID・名称・目的・由来・生成関数は維持し、中盤2・終盤2を加えて7局面にした。
- 固定手順は公開`getLegalActions()`で意味的にちょうど1手へ照合し、`executeLegalAction()`で適用する。駒打ちも扱える狭い`BookAction` unionへ拡張し、再生後の実手番を期待値と照合する。局面キーの決定性・重複なし、active/check、合法手あり、後手番ありはテストで検証する。
- 比較結果・JSON・テキストに`phase`と実手番を追加した。比較の3 warmup/8 measurement、交互順、中央値、深さ分布、失敗継続は変更していない。7局面×22呼出しは探索本体ではなくfixture runnerで検証する。

### 範囲・制限

- 中盤は静かな駒組みと飛車先の取り返し、終盤は王手回避と持ち駒の歩打ち候補を分けた。追加局面は決定的な公開API再生の研究用fixtureであり、実戦由来と主張していない。
- 今回は5秒・10秒の全局面実測を行わない。性能改善、棋力/Elo、統計的有意差、キラームーブ既定ON、通常対局、Worker、UI、評価・SEE・静止探索・αβ本体、保存形式、CI性能閾値、依存関係は変更・結論しない。
...
## [2026-09-23 JST] キラームーブ比較の可変時間枠（5秒・10秒）

### 設計・検証

- `origin/main`を許可経路でfetchし、HEAD/`origin/main`ともPR #140のマージコミット`f0068d0`であること、作業ツリーが空であることを確認して`feat/killer-move-time-budget-comparison`を作成した。親階層・リポジトリ内に追加の`AGENTS.md`はなかった。
- `TimedSearchOptions`を呼出しごとの不変値として追加し、timed探索へ最大深さ・時間制限を渡す。既定は従来どおり1,000ms・最大深さ4で、グローバルな可変状態は持たない。fixedは通常深さ3のままでtimed専用オプションを拒否する。
- キラー専用CLI解析は共有の`parseSuiteComparisonMode`を変えず、新しい純粋関数に隔離した。値なし、0、負数、小数、NaN、Infinity、非数字、重複、未知オプション、最大深さ1未満、fixedとの混在を測定開始前に使用例付きで拒否する。
- 本測定だけから正の整数完了深さを昇順集計し、failed/nullを別深さに変換しない。JSONにはconfig、両設定、全試行の生データ、局面別の深さ分布を、テキストには同じ設定と分布を残す。ON/OFF間で異なる設定は`killerMoves`だけである。

| コマンド | 終了コード | 結果 |
| --- | ---: | --- |
| `npm test -- src/test/quiescence-ordering-suite-comparison.test.ts` | 0 | 14/14成功。CLI、既定、5秒・10秒伝播、順序、拒否、設定差、不変性、分布、JSON/テキストを決定的に検証 |
| `npm run lint` | 0 | TypeScriptエラー0 |
| `npm test` | 0 | 59ファイル1723/1723成功 |
| `npm run build` | 0 | 1755 modules、build成功 |
| `npm run check` | 0 | lockfile、lint、59ファイル1723/1723テスト、build成功 |
| `git diff --check` | 0 | 空白エラー0 |

### 実測

- 実装・検証後に、他のテスト・build・測定を並行させず、固定3局面、ウォームアップ3回、本測定8回、OFF/ON交互・先行各4回、最大深さ4を維持して5秒、10秒の順で各1回だけ実行した。生データはCLI JSON出力に保持され、集計は`docs/killer-move-suite-comparison.md`へ記録した。
- 5秒（開始02:26:21Z、終了02:31:54Z、終了0）は正常3/3・エラー0。平手はOFF深さ2×7/3×1、ON深さ3×8、定跡分岐はOFF深さ2×7/3×1、ON深さ2×8、飛車先はOFF深さ2×8、ON深さ3×8だった。飛車先ではONだけが深さ3を完了し、手・評価は`▲7六歩/214→▲2四歩/-1`へ変化した。平手では双方が深さ3を完了した。
- 10秒（開始02:32:42Z、終了02:43:39Z、終了0）は正常3/3・エラー0。平手・定跡分岐は双方深さ3×8、飛車先はOFF深さ2×6/3×1/4×1、ON深さ3×8だった。ONだけが深さ3を完了した局面はなく、全局面で双方が少なくとも1回は深さ3を完了した。手・評価の変化は同じ飛車先だけだった。
- この固定3局面・同一実行環境の反復から、棋力、Elo、統計的有意差、一般的な性能改善、既定ON化、UI・通常対局への採用を結論しない。次候補は中盤・終盤を含む追加局面への拡張であり、今回のPRには局面追加を含めない。
## [2026-09-23 JST] 時間制限付き通常探索の実戦系ストレス測定

- 基点はPR #148を含む`main`/`origin/main`の`64586f0`、開始時の作業ツリーは空。`feat/timed-search-stress-benchmark`で作業。探索本体、時間制御、Worker/UI、結果型、対局保存は変更しない。
- 既存`scripts/measure-time-limit-overrun.ts`の同期直列、各条件ウォームアップ2回＋本測定5回、最大深さ4、100/1,000msを踏襲。`scripts/benchmarks/timeLimitStressPositions.ts`は保存済み99手対局の合法手列から50/98手を再生し、別途seed=1の合法手プレイアウトを150手再生。既存の飛車先取り返し10手・王手回避構成局面を加え、由来を分離する。抽出99手は元`docs/quiescence-ordering-self-play-output.txt`の最初の`RECORD`とテストで完全一致を検証。元ファイルSHA-256=`9cbb47363d718cf6f550a7689c5e23090550bf3cd4db7321b8243d15ce064cb1`、抽出ファイルSHA-256=`49a48da6108811946090c1c0d01e5ae4b3078d0779da6177269d7164bffb0956`。
- 5局面は順に履歴50/98/150/10/0、root合法手136/290/90/49/11、盤上駒34/21/36/36/8、固定深さ1＋静止追加1手の静止訪問15/10/123/65/28。50/98は同じ実対局の相関した局面、150は実戦棋譜と称さない決定的対局、10手は固定手順、王手回避は構成局面。すべてrootで合法手があり、未終局。履歴を偽装しない。
- JSONLは`docs/benchmarks/time-limit-stress-20260923.jsonl`。`config`1、`position`5、`sample`140（ウォームアップ40、本測定100）、100ms`summary`10、`end`1行。各sampleに局面ID/由来/履歴手数/root合法手数/モード/phase/run/指定時間/実経過/API経過/完了深さ/timeout/通常・静止訪問数/両合計/深さ1経過/選択手を保存。通常・静止の訪問数は最深完了反復、合計は完了済み反復のみ。未完了反復の訪問は含まず、経過時間には含む。
- 実測環境はWindows 10.0.26200 x64、Intel Core i7-14650HX、Node v24.20.0。開始2026-09-23T13:52:11.327Z、同期直列。テスト・build・他ベンチマークと並走させず、終了0。探索呼出しの直前・直後だけを単調時計で測り、局面生成・静止訪問probe・ウォームアップを各本測定の時間から外した。
- 100ms本測定の最大/中央値(ms)、105ms超回数、300ms以上回数はREADME表のとおり。保存済み実対局50手の静止探索ありは5/5が344.20～366.78ms、300ms以上が5/5、500ms以上は0。最長回は実経過366.78ms、深さ1反復366.60ms、完了深さ1・timeout=true。ほかの105ms超もすべて完了深さ1で、深さ1が100msを超過していた。105ms超の原因分類は深さ1最低保証まで証拠があり、深さ1内部の費用内訳は未測定。深さ2以降の協調的期限確認間隔による大幅超過は今回の標本では示されない。
- 1,000ms本測定50件は全件timeout、実経過1000.25～1005.21ms、深さ1または2まで完了。100msのしきい値は厳密保証の合否判定に用いない。1台の5回反復・同一対局の2局面だけで一般化せず、次課題は深さ1保証、Worker/UI、結果型、対局記録を含む厳密な時間制御の設計とする。
- 再実行: `npm run measure:time-limit-stress -- docs/benchmarks/time-limit-stress-local.jsonl`。出力先は上書きされる。関連テスト4ファイル44件、変更途中の`npm run check`はlockfile、lint、61ファイル1729件、build成功。抽出データ一致テスト追加後の最終check結果は別途記録する。
- 抽出データ一致テスト追加直後の`npm run check`は1件失敗（`import.meta.url`がVitest変換URLで`file:`ではなかった）。テストの参照先をリポジトリrootの`docs/...`へ修正し、製品コード・測定スクリプト・JSONLは変更していない。修正後`npm run check`終了0、lockfile検証・lint・61ファイル1730/1730テスト・build（1755 modules）成功。既存のjsdom navigation未実装通知1件は失敗ではない。
