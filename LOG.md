...
- スモーク: 100ms・1ペア2局・4手上限、8合法着手、fallback 0、打切2、実行913ms。主条件: 100ms・2ペア4局・120手上限、421着手、実行84.2秒。fallback A 194/211、B 195/210、計389/421。千日手引分1（61手）、打切3（各120手）、勝敗0。1～20手60/80、21～40手73/80、41～60手77/80、61～80手61/61、81～100手58/60、101～120手60/60。実時間は100ms超421/421、105ms超259/421、200ms超141/421、最大398.55ms。API内最大398.50ms。
- 参考条件: 1,000ms・1ペア2局・40手上限、80着手、実行82.0秒。fallback A 6/40、B 7/40、計13/80。1～20手0/40、21～40手13/40。打切2、勝敗・引分0。実時間は1,000ms超80/80、1,005ms超9/80、1,100ms超0、最大1,009.87ms。API内最大1,009.82ms。3条件のJSONL全509着手の合法性と保存済み集計をauditで確認した。
- 追加テスト3件を許可経路で実行し成功。通常サンドボックスではVitest起動時にesbuild `spawn EPERM`で未実行。`npm run check`は許可経路で終了0。lockfile検証、TypeScript型検査、62ファイル1738/1738テスト、Vite build成功。既存jsdomのnavigation通知はテスト失敗ではない。100msの全局にfallbackがあり3/4局打切、1,000msも2/2局打切なので成績との関連を判定できない。旧版比較は深さ1完走による実時間不一致があり、この計測には含めず、隔離した実時間統制対局を別課題とする。
