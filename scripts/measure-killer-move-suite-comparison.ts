...
import { defaultKillerMoveSuiteComparisonConfig, runSuiteComparisonCli } from './benchmarks/quiescenceOrderingSuiteComparison';
...
try {
  ...
  console.log(`RUN ${rawJson({ startedAt, node: process.version, npm: process.env.npm_config_user_agent ?? 'unknown',
    ...
    command: 'npm run measure:killer-move-suite-comparison -- [fixed|timed]',
    sourceSha256: Object.fromEntries(sourceFiles.map((path) => [path, createHash('sha256').update(readFileSync(path)).digest('hex')])),
  })}`);
  process.exitCode = runSuiteComparisonCli(process.argv.slice(2), {}, undefined, console.log, defaultKillerMoveSuiteComparisonConfig);
} catch (error) {
