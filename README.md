...
再生した状態は実対局と同じ`protectSearchInput`で複製し、全階層をProxyで保護して探索に渡します。この準備は呼び出し時計の外です。`SearchDiagnostics`は任意指定時だけ有効です。root合法手生成、通常探索の合法手生成、並べ替え、SEE、着手適用、通常探索その他、静止探索を測ります。入れ子の時間は外側から差し引き、静止探索にはその内部の合法手生成・評価・着手適用を含めます。APIその他は診断開始から終了までの未分類残差です。工程タイマーの読み取り負荷は主に外側の区分に入り、診断OFF/ON比較で測ります。100msをまたいだ工程と、その後の協調的確認で中断を検知した工程は別フィールドです。標準手順ではSEEを呼びません。
...
本測定はコード`f7ae3d2ffe4c460aa3ef1f2dc09deb12d4301caa`、クリーンな独立作業ツリーで実施しました。Windows 10.0.26200 x64、Intel Core i7-14650HX、Node v24.20.0です。[生データ](docs/benchmarks/timed-depth-one-main-20260924.jsonl)と[生成集計](docs/benchmarks/timed-depth-one-main-20260924.md)を保存し、196サンプルと集計を`audit:timed-depth-one`で再照合しました。2回のウォームアップを集計から除外し、各局面・条件5回、計35回ずつを本測定としました。中央値は最速値でなく本測定の中央値です。初回の通常状態を直接渡した測定では実対局のProxy保護が欠けており、主結果には採用しません。

| 局面帯 | 局面数 | 100ms深さ1完走（診断OFF/ON） | fallback（OFF/ON） | 診断ONの固定深さ1中央値ms | 診断ONの静止探索中央値ms（100ms探索） |
| --- | ---: | ---: | ---: | ---: | ---: |
| 序盤 | 2 | 5/10・5/10 | 5/10・5/10 | 35.60 | 36.30 |
| 中盤 | 2 | 5/10・5/10 | 5/10・5/10 | 77.51 | 0.00 |
| 後半 | 3 | 5/15・5/15 | 10/15・10/15 | 372.35 | 41.50 |

固定深さ1は`g1-p30`で診断ON447～511ms、`g3-p55`で1974～2752ms、`g1-p115`で771～808msかかりました。`g1-p115`は314個のroot候補ごとに静止探索を呼び、診断ONでは静止探索が569～597ms、通常着手適用が152～161msです。`g3-p55`は222候補、`g4-p89`は249候補を処理します。多数のroot候補に対する着手適用と静止探索が、深さ1の100ms以内の完走を妨げる可能性が高いです。root合法手の取得自体が100msを超える回もあります。

診断ONの100ms本測定35回で、100msをまたいだ区分はroot合法手生成10回、静止探索16回、通常合法手生成6回、通常着手適用3回でした。中断を検知した区分はAPIその他10回、静止探索7回、通常探索その他18回です。中盤の静止探索中央値0msは、片方の局面でroot合法手生成だけで期限を過ぎ、静止探索へ進まない5回を含むためです。root合法手生成が100msを超えた`g3-p55`は243～263ms、`g4-p89`は350～399msの呼び出し実時間で、探索開始前の1処理を中断できないため超過しました。`g1-p115`は元棋譜で333.57msでしたが、今回の100ms探索は100.3～102.7msでfallbackし、元の単発超過がどの工程で生じたかは特定できません。ただし同じ種類の大幅超過は`g3-p55`と`g4-p89`のroot合法手生成中に再現しました。通常並べ替えは小さく、SEEは標準設定で0回です。100ms測定の工程時間には中断した深さ2以降も含まれ、深さ1だけの費用は固定深さ1の行で確認します。工程別中央値は別々に計算しており、足し算して全体中央値とは一致しません。

診断OFF/ONを同一局面・反復番号で比較した実時間差は、固定深さ1で中央値-0.82ms（範囲-697.35～+39.94ms）、100ms探索で中央値+0.03ms（範囲-72.36～+67.12ms）でした。固定深さ1の本測定70回は選択手・評価・PVが一致しました。100ms探索では同じ35組中1組で完了深さが1段違い、選択手は全組一致しました。診断負荷を時間差だけから精密に見積もるにはOS・JIT・GCの揺れが大きく、100msでの深さは1組揺れました。それでも診断OFFでも中盤最大286.67ms、後半最大422.29msとなり、root合法手生成の中断不能区間と固定深さ1の高い費用という結論は診断ONだけに依存しません。元棋譜の443/480というfallback率は選別した7局面の反復率とは直接比較できません。後続では長い連続対局で各手のヒープ・GCイベントとroot合法手生成／静止探索内部の最大連続処理時間を記録し、特に`g1-p115`の長い尾を調べます。この診断から棋力や勝率は判断しません。PR #154の全局は手数上限で打切です。
