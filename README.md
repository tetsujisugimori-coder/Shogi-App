...
- **将棋ドメイン層・合法手エンジン**:
  ...
  - **二系統の着手実行方針**:
    - **アシスト方式（Human UI）**: 禁じ手を候補から除外し、不正手を安全に拒否（対局は継続）
    - **厳格対局方式（AI・将棋エンジン実験）**: 禁じ手を提案手として受け取り、盤面を変更せず反則負け（`foul_loss`）として記録
  - イミュータブルな局面更新（`applyLegalMove`）、手番交代、手数加算、駒捕獲（成駒の成解除）、棋譜生成（`▲7六歩`等）
